...
export function RoleMatrixPanel() {
  ...
  const [editingRole, setEditingRole] = useState<AccountRole>('業務')
  const countBy = (role: AccountRole, test: (access: DocAccess) => boolean) =>
    DOC_KEYS.filter((doc) => test(docPermission(doc, role).access)).length
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          調整「誰可以對哪張單做哪件事、看到哪些欄位」。設定對象是角色，改一次全體生效。
        </p>
      ...
      <div className="mb-4 rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
        ...
        <ul className="mt-1 list-disc space-y-0.5 pl-5 text-xs">
          ...
          <li>
            ...
            要確實禁止某個人，請用
            <Link to="/masters/accounts?tab=exclusions" className="text-brand hover:underline">
              個別排除
            </Link>
          </li>
          <li>
        ...
        <CardContent>
          <Tabs defaultValue="by-role">
            <TabsList className="mb-4">
              <TabsTrigger value="by-role">依角色設定</TabsTrigger>
              <TabsTrigger value="overview">總覽（單據 × 角色）</TabsTrigger>
            </TabsList>

            {/* 依角色設定：先選角色，再逐張單據設定「看不看得到」與「可以做哪些事」 */}
            <TabsContent value="by-role">
              <div className="mb-3 flex flex-wrap gap-2">
                {ACCOUNT_ROLES.map((role) => (
                  <button
                    ...
                    className={
                      editingRole === role
                        ? 'rounded-md border border-brand bg-brand px-3 py-1.5 text-sm text-white'
                        : 'rounded-md border border-border bg-surface px-3 py-1.5 text-sm text-ink-body hover:bg-muted'
                    }
                  >
                    {role}
                    <span className="ml-1.5 text-xs opacity-80">
                      {countBy(role, (a) => a !== '無權限')}/{DOC_KEYS.length}
                    </span>
                  </button>
                ))}
              </div>
              ...
              <p className="mb-3 text-sm text-ink-body">
                「{editingRole}」可看到 <b>{countBy(editingRole, (a) => a !== '無權限')}</b> 張單據（可操作{' '}
                {countBy(editingRole, (a) => a === '可操作')}、唯讀 {countBy(editingRole, (a) => a === '唯讀')}）。
                設為「無權限」的單據不出現在側欄，網址直接進入也會擋下。
              </p>
                ...
                <table className="w-full min-w-[720px] text-sm">
                  <thead>
                    <tr className="border-b border-border text-left">
                      ...
                      <th className="px-2 py-2 font-medium text-muted-foreground">檢視範圍</th>
                      <th className="px-2 py-2 font-medium text-muted-foreground">可執行的動作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {DOC_KEYS.map((doc) => {
                      const perm = docPermission(doc, editingRole)
                      const changed = isDocPermissionOverridden(doc, editingRole)
                      const hidden = perm.access === '無權限'
                      return (
                        <tr
                          key={doc}
                          className={changed ? 'border-b border-border/60 bg-warning/10' : 'border-b border-border/60'}
                        >
                          <td
                            className={
                              hidden ? 'py-2 pr-3 font-medium text-muted-foreground' : 'py-2 pr-3 font-medium text-ink'
                            }
                          >
                            {DOC_LABELS[doc]}
                          </td>
                            ...
                            <div className="inline-flex overflow-hidden rounded-md border border-border">
                              {ACCESS_OPTIONS.map((o) => (
                                <button
                                  ...
                                  title={o.hint}
                                  onClick={() => {
                                    if (perm.access === o.value) return
                                    guard(
                                      () => setRoleDocAccess(account, doc, editingRole, o.value),
                                      '已將「' + editingRole + '」在' + DOC_LABELS[doc] + '改為「' + o.value + '」',
                                    )
                                  }}
                                  className={
                                    perm.access === o.value
                                      ? 'border-r border-border bg-brand px-2.5 py-1 text-xs text-white last:border-r-0'
                                      : 'border-r border-border bg-surface px-2.5 py-1 text-xs text-ink-body last:border-r-0 hover:bg-muted'
                                  }
                                >
                                  {o.label} {o.value}
                                </button>
                          ...
                          <td className="px-2 py-2">
                            <div className="flex flex-wrap gap-x-3 gap-y-1">
                              {DOC_ACTIONS[doc].map((action) => (
                                <label
                                  key={action}
                                  className={
                                    hidden
                                      ? 'inline-flex items-center gap-1 text-xs text-muted-foreground'
                                      : 'inline-flex items-center gap-1 text-xs text-ink-body'
                                  }
                                >
                                  <input
                                    type="checkbox"
                                    ...
                                    onChange={(e) =>
                                      guard(
                                        () => setRoleDocAction(account, doc, editingRole, action, e.target.checked),
                                        '已' +
                                          (e.target.checked ? '開放' : '取消') +
                                          '「' +
                                          editingRole +
                                          '」在' +
                                          DOC_LABELS[doc] +
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                勾選任一動作會自動把該單提升為「● 可操作」；改為唯讀或無權限時，該角色在這張單上的動作一併清空。
                以底色標示者與出廠預設不同。
              </p>
            </TabsContent>
