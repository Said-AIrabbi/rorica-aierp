  ...
  return (
    <div>
      <PageHeader
        ...
        description="點選任一列可開啟編輯視窗，該產品分支底下實際存在的每一捲布（布卷資料）列於詳細頁。產品編號為皇加編碼「類別-流水號」（如 1-11 ＝ 第一類緞布的第 11 個產品），建檔時依所選類別自動給下一號，與產品序號（分支）皆由系統維護、不可修改；同一皇加品名的規格若有些微差異，會各自建檔並以產品序號區分分支。顏色為「客戶＋皇加品名＋色號＋染整廠」四者綁定，非通用色號。標示 ⚠ 表示超過12個月未使用，疑似覆色。進價/售價欄位可見範圍待依角色權限另行設定，此處先顯示全部。"
      />
      <DataTable
        columns={columns}
        data={data}
