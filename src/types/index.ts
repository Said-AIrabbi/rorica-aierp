...
export interface Product {
  /**
   * 產品編號（主鍵）：皇加既有的編碼方式「**產品類別-流水號**」，如 1-11 ＝ 第一類緞布的第 11 個產品。
   * 建檔時由系統依所選類別自動產生（該類別現有最大流水號加一），不開放修改，各單據一律以此關聯商品。
   *
   * 系統不另設一組內部主鍵：產品編號本身即唯一識別碼，畫面上看到的編號與資料關聯用的是同一個，
   * 不會出現「報給客戶的編號」與「系統裡的編號」兩套。
   *
