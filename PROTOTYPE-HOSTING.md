...
單據異動通知（主 PRD 決策128）要能到達「別人」，畫面就得知道伺服器上已經變了。
原型沒有可以推播的後端行程，故改為**每 15 秒問一次版本號**：版本沒變就什麼都不做，
變了才把整包資料換掉並讓畫面重新查詢。

三種情況跳過該輪，免得洗掉使用者還沒送出的東西：已進入衝突凍結、自己正在寫或還有待寫、分頁在背景。

代價是**最多 15 秒的延遲**，且對方的異動會整包蓋掉本機那份——這在「同一份資料、各自操作」的
展示情境下是對的，真後端則應改為單筆更新與事件推送。
...
- **通知靠輪詢**，最多慢 15 秒；分頁切到背景時不輪詢，切回來才補上。
